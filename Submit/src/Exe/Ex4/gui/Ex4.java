	...
	private static void drawShape(GUI_Shapeable g) {
		...
		if(gs!=null) {
			...
			if (gs instanceof Rect2D erect) {
				...
				double[] x ={rect.x(),angle.x(),p3.x(),p4.x()} ; // stores all the x coordinate of each point
				double[] y = {rect.y(),angle.y(),p3.y(),p4.y()}; // stores all the y coordinate of each point
				if (isFill) {
					StdDraw_Ex4.filledPolygon(x,y);
				} else {
					StdDraw_Ex4.polygon(x,y);
	...
	private void copy( Point2D vector) {
		for(int i=0;i<_shapes.size();i++) {// runs on the entire array
			...
			if(s.isSelected() && g!=null) {// checks if the object was slected
				...
				this._tag++;
				GUI_Shapeable gs = new GUIShape(copy,s.isFilled(), s.getColor(), this._tag);// defines a new GUI_shapeable
				s= gs;// stores into s
				this._shapes.add(gs);
