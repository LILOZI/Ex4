...
public class Point2D{
	...
	public void rotate(Point2D cen, double angleDegrees) {
        ...
        double x = (_x - cen.x()) * Math.cos(angleRadians) - (_y - cen.y()) * Math.sin(angleRadians) + cen.x();
        double y = (_x - cen.x()) * Math.sin(angleRadians) + (_y - cen.y()) * Math.cos(angleRadians) + cen.y();
        x=Double.parseDouble(String.format("%.5g%n", x)); // this rounds the answer don't know what you prefer
        y=Double.parseDouble(String.format("%.5g%n", y));
        this._x = x;
        this._y = y;
	}
